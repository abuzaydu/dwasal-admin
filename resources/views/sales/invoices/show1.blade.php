  ...
  <div class="row">
        <div class="col-md-12 mx-auto">
            <ul class="nav nav-tabs nav-tabs-new2">
                ...
                @endif
                <!-- <li class="nav-item">
                    <a class="nav-link" data-bs-toggle="tab" href="#sales-order-rview">Sales Order Receipt View</a>
                </li> -->
                @if(Auth::user()->can('view-sales-order'))
                <li class="nav-item">
                    <a class="nav-link" data-bs-toggle="tab" href="#sales-order"><i class='fa fa-list-plus font-18 me-1'></i> Sales Order</a>
                </li>
                @endif
            </ul>
            <?php 
            ...
            <div class="tab-content py-3">
                <div class="tab-pane fade" id="sales-order" role="tabpanel">
                     <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-12">
                                    @if(Auth::user()->can('print-sales-order'))
                                    <button onclick="javascript:saveOrderPdf()" type="button" class="btn btn-outline-success btn-sm" style="margin-left: 2px; padding-top: 5px;">
                                        <i class="bx bx-download"></i> {{trans('navmenu.download')}} PDF
                                    </button>
                                    @endif
                                    @if($settings->allow_initiate_sto_from_invoice)
                                    @if(Auth::user()->can('create-stock-transfer') && !$sale->is_stock_requested)
                                    <form method="POST" action="{{ url('create-st-request') }}" style="display: inline;">
                                        @csrf
                                        <input type="hidden" name="id" value="{{$sale->id}}">

                                        <button type="submit" class="btn btn-success btn-sm"><i class="bx bx-file"></i>New STO Request</button>
                                    </form>
                                    @endif
                                    @endif
                                </div>
                            ...
                            <div class="row g-1 print_invoice" id="print-order">
                                <div class="col-md-12">
                                    <table class="table mb-1">
                                        <tbody>
                                            <tr>
                                                <td style="text-align: center; background: <?php echo $settings->invoice_color; ?>;">
                                                    <h6 class="mb-0 text-uppercase" style="color: <?php echo $settings->invoice_title_color; ?>;">POS</h6>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                                <div class="col-md-12 border-bottom pb-0" style="border-bottom: 1px solid black;">
                                    <table class="items mt-0">
                                        <tr>
                                            <td style="width: 60%;padding-left: 30px;">
                                                @if(!is_null($company->logo_url))
                                                <figure>
                                                    <img class="invoice-logo" src="{{asset('storage/clogos/'.$company->logo_url)}}" alt="" width="250" style="border: 1px solid white;">
                                                </figure>
                                                @endif
                                            ...
                                            <td style="width: 40%;">
                                                <table class="meta mt-0">
                                                    <tbody>
                                                        <tr>
                                                            <td class="meta-head" style="font-size: 14px !important;">Order No.</td>
                                                            <td style="font-size: 14px !important;"><b>{{ sprintf('%06d', $sale->invoice_no)}}</b></td>
                                                        </tr>
                                                        <tr>
                                                            <td class="meta-head" style="font-size: 14px !important;">Order Date</td>
                                                            <td style="font-size: 14px !important;"><b id="date">{{ date('d F, Y H:i:s:A', strtotime($sale->time_created)) }}</b></td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </td>
                                ...
                                <div class="col-md-12">
                                    <table class="list-items mt-2">
                                        <thead>
                                            <tr>
                                                <th class="desc" style="width: 79%;">Description</th>
                                                <th char="qty" style="text-align: center; width: 8%;">Qty</th>
                                                @if(Auth::user()->can('view-order-amount'))
                                                @if($settings->show_discounts)
                                                <th style="text-align: center; width: 8%;">Disc @if($settings->discount_by_percent)(%)@endif</th>
                                                @endif
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                            <tr>
                                                <td class="desc" style="">@if(!is_null($item->product_code)){{$item->product_code}} - @endif {{$item->name}}</td>
                                                <td class="qty" style="text-align: center;">{{number_format($quantity_sold + 0)}} {{$punit->unit_name}}</td>
                                                @if($settings->show_discounts)
                                                ...
                                                <td class="total" style="text-align: right;">{{number_format(($item->price-$item->total_discount)*$ex_rate, 2, '.', ',')}}</td>
                                                @else

                                                <td class="total" style="text-align: right;">{{number_format(($item->price)*$ex_rate, 2, '.', ',')}}</td>
                                                @endif
                                            </tr>
                                            @endforeach
                                            ...
                                            <tr>
                                                ...
                                                <td style="text-align: right;">
                                                    {{number_format($item->total-$item->total_discount, 2, '.', ',')}}</td>
                                                @else
                                                <td style="text-align: right;">
                                                    {{number_format($item->total-$item->total_discount, 2, '.', ',')}}</td>
                                                @endif
                                    ...
                                    <table class="mt-0" style="width: 100%;">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="unit" colspan="2" style="text-align: right;"><b>{{trans('navmenu.total')}} (Excl.Tax):</b></td>
                                            ...
                                            <tr>
                                                <td style="border: none;"></td>
                                                <td class="unit" colspan="2" style="text-align: right;"><b>{{trans('navmenu.vat')}} ({{number_format($settings->tax_rate)}}%)</b>:</td>
                                                <td class="total" style="text-align: right;"><b>{{number_format($sale->tax_amount*$ex_rate, 2, '.', ',')}}</b></td>
                                            </tr>
                                            @endif
                                            <tr>
                                                <td style="border: none;"></td>
                                                <th colspan="2" class="unit" style="text-align: right; border-bottom: 1px solid black;"><b>{{trans('navmenu.total')}} (Inc.Tax) ({{$stmtcurr}})</b>:</th>
                                                <th class="total" style="text-align: right; border-bottom: 1px solid black;"><b>{{number_format((($sale->sale_amount-$sale->sale_discount)+$sale->tax_amount)*$ex_rate, 2, '.', ',')}}</b></th>
                                            </tr>
                                            ...
                                            <tr>
                                                <td style="border: none;"></td>
                                                <td class="unit" colspan="2" style="text-align: right;"><b>{{trans('navmenu.discount')}}:</b></td>
                                            ...
                                            <tr>
                                                <td style="border: none;"></td>
                                                <td colspan="2" style="text-align: right;"><b>{{trans('navmenu.vat')}} ({{number_format($settings->tax_rate)}}%)</b>:</td>
                                                <td class="total" style="text-align: right;"><b>{{number_format($sale->tax_amount*$ex_rate, 2, '.', ',')}}</b></td>
                                            </tr>
                                            @endif
                                            <tr>
                                ...
                                <div class="col-md-12" style="text-align: center; margin-top: 5px; padding: 15">
                                    <small style="font-size: 8px;">{{trans('navmenu.issued_by')}}: <strong>{{Auth::user()->first_name}} {{Auth::user()->last_name}}</strong></small>
                                </div>
                            </div>
                            <div id="editor"></div>
                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td style="width: 50%; padding-right: 20px;">
                                                <table class="meta">
                                                    <tbody>
                                                        <tr>
                                                            <td style="border: none; text-align: right; font-size: 16px !important;" class="meta-head">Invoice No. : <b>{{ sprintf('%06d', $sale->invoice_no)}}</b></td>
                                                        </tr>
                                                        ...
                                                        @endif
                                                        <tr>
                                                            <td style="border: none; text-align: right;" class="meta-head">Issue Date : <b id="date">{{ date('d F, Y', strtotime($sale->time_created)) }}</b></td>
                                                        </tr>
                                                        <tr>
