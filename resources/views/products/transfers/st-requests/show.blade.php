            ...
            <div class="card">
                <div class="card-body">
                    <div class="border rounded p-2 mb-1">
                        @if(Auth::user()->can('receive-stock-transfer') || Auth::user()->can('receive-returned-stock') && is_null($transorder->receive_time))
                        @if($transorder->is_request && $transorder->status == 'Confirmed')
                        ...
                        <a onclick="confirmReceive()" class="btn btn-outline-success btn-sm" style="margin: 5px;"><i class="fa fa-check"></i> Receive Order</a>
                        @endif
                        @endif
                        @if($transorder->status == 'Pending')
                        ...
                        @endif
                        @if(Auth::user()->can('view-sto-value'))
                    ...
                    <form class="form row g-1" id="receive-form" method="POST" action="{{route('transfer-orders.update', encrypt($transorder->id))}}" style="display: inline;">
                        @csrf
                        {{ method_field('PATCH') }} 
                        ...
                        <input type="hidden" name="add_vat" value="{{$transorder->add_vat}}">
                            ...
                            <table class="list-items mt-0" style="width: 100%;">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @if($transorder->is_request)
                                        <td style="text-align: center;">{{$item->req_qty+0}} {{$item->basic_uom}}</td>
                                        @endif
                                        <td style="text-align: center;">{{$item->quantity+0}} {{$item->basic_uom}}</td>
                                ...
                                <table>
                                    <tr>
                                        <td style="font-size: 8px !important;">
                                            ...
                                            Date <strong>@if(!is_null($transorder->confirm_time)){{date('d M Y H:i A', strtotime($transorder->confirm_time))}}@endif</strong><br>
                                            {{trans('navmenu.signature')}} <strong>.....................</strong>
                                        </td>
                                    </tr>
                                </table>                  
...
@endsection
    ...
    <script language="javascript" type="text/javascript">
        function confirmReceive() {
            Swal.fire({
              title: "Are you sure you receied the order?",
              text: "{{trans('navmenu.no_revert')}}",
              icon: 'warning',
              showCancelButton: true,
              confirmButtonColor: '#3085d6',
              cancelButtonColor: '#d33',
              confirmButtonText: "Yes receied",
