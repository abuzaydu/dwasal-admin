            ...
            <div class="col-lg-4 col-md-4 col-sm-12 text-right">
                            ...
                            <div class="btn-group float-end" role="group">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-end"> 
                                    ...
                                    <form class="row g-3" method="POST" action="{{ url('pt-sto-request') }}" id="ptemp-form-{{$key}}">
                                        @csrf
                ...
                <div class="card-body">
                    ...
                    <div class="p-4 border rounded">
                        <form class="row g-3 needs-validation" novalidate name="orderform" method="POST" action="{{route('transfer-orders.store')}}" onsubmit="return validateform(this)">
                            ...
                            <div class="col-sm-3">
                                <label class="form-label">Transfer Type<span style="color: red; font-weight: bold;">*</span></label>
                                <select name="transfer_type" ng-model="ordertemp.transfer_type" ng-change="updateTransferOrderTemp(ordertemp)" ng-options="mtype.id as mtype.type for mtype in types" class="form-select form-select-sm mb-1" required>
                                </select>
                            </div>
                            <div class="col-sm-5">
                                <label for="destin_id" class="form-label">Warehouse <span style="color: red; font-weight: bold;">*</span></label>
                            ...
                            <div class="col-sm-6">
                                    ...
                                    <div class="input-group mb-0">
                                        ...
                                        <!-- <button class="btn btn-outline-primary btn-sm" type="submit" id="button-addon2"><i class='fa fa-search'></i> Search</button> -->
                                        <a class="btn btn-outline-danger empty-search btn-sm" id="button-addon2"><i class='fa fa-close'></i></a>
                                    </div>
                                    <ul id="searchResult3" class="list-group"></ul>
                                </div>
                            <div class="col-md-12 pt-3" style="margin-top: 5px; border-top: 2px solid #BBDEFB;">
                                <table class="table" style="width: 100%;">
                                    ...
                                    <tbody>
                                        <tr ng-repeat="neworderitemtemp in orderitemtemp" id="temps">
                                            <td>@{{$index + 1}}</td>
                                            <td class="item">@{{neworderitemtemp.product_code}} @{{neworderitemtemp.slug}}</td>
                            ...
                            <div class="col-sm-7">
                                ...
                                @if(is_null($reason))
                                <textarea name="reason" rows="1" class="form-control form-control-sm mb-1"></textarea>
                                @else
                                <input type="text" name="reason" ng-model="ordertemp.reason" ng-blur="updateTransferOrderTemp(ordertemp)" class="form-control form-control-sm mb-1" value="{{$reason}}">
                                @endif
                            </div>
                            <div class="col-md-12">  
                                <button type="submit" class="btn btn-success btn-sm mb-1"><i class="fa fa-plus" ></i>{{trans('navmenu.create_order')}} </button>
                                <a href="{{url('cancel-temp-order/'.encrypt($ordertemp->id))}}" class="btn btn-warning btn-sm mr-1 card-subtitle" id="btn-cancel"><i class="fa fa-x"></i>{{trans('navmenu.cancel_order')}}</a>
                            </div>
                        </div>
                    </form> 
                </div>
    ...
    <script type="text/javascript">
        ...
        function addOrderItemTemp(element) {
            ...
            $.ajax({
                ...
                success:function (response) {
                    var item = response;
                    angular.element(document.getElementById('mycontroller')).scope().addSTOItemTemp(item);
                    setTimeout(function(){
                        $("#search_key").val('');
                        $("#searchResult3").empty();
                    }, 2000);
