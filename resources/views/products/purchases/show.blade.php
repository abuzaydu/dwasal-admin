                                ...
                                <tr>
                                    ...
                                    <td style="width: 50%; border: 1px solid gray;">
                                        ...
                                        <small style="font-size: 8px">
                                            @if(!is_null($supplier->address)){{$supplier->address}}<br>@endif @if(!is_null($supplier->phone))Mobile : <a href="#">{{$supplier->phone}} </a>@endif @if(!is_null($supplier->email))Email :<a href="#" style="text-transform: lowercase;">{{$supplier->email}}</a><br>@endif @if(!is_null($supplier->tin)) TIN : {{$supplier->tin}}@endif @if(!is_null($supplier->vrn))VRN : {{$supplier->vrn}}<br>@endif
                                        </small>
                                    </td>
                                </tr>
                            </table>
                        ...
                        <div class="col-md-12">
                            <table class="list-items mt-0" style="width: 100%;">
                                ...
                                <tbody>
                                    ...
                                    <?php $tqty += $item->quantity_in; ?>
                                    <tr>
                                        <td class="no">{{$key+1}}</td>
                                        <td class="text-left">{{$item->name}}</td>
                                        <td style="text-align: center;">{{$item->basic_uom}}</td>
                                        <td style="text-align: center;">{{$item->quantity_in+0 }}</td>
                                        @if($shop->business_type_id != 1)
                                        <td style="text-align: center;">{{number_format($item->unit_cost)}}</td>  
                                        <td style="text-align: right;">{{number_format($item->quantity_in*$item->unit_cost)}}</td>
                                        @endif
                                    </tr>
                                    @endforeach
                                </tbody>
