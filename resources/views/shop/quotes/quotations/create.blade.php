                ...
                <div class="card-body">
                    <form action="{{ route('quotations.store') }}" method="POST" id="quotationForm">
                        ...
                        <div class="text-end">
                            ...
                            <button type="submit" class="btn btn-primary" id="saveBtn">Save Quotation</button>
                        </div>
                    </form>
                </div>
            </div>
...
@section('page-scripts')
    <script>
        $(function() {
            ...
            function buildUnitOptions(units, selectedUnitId) {
                if (!units || !units.length) {
                    return '<option value="">N/A</option>';
                }
                return units.map(function(u) {
                    var selected = (selectedUnitId && u.id == selectedUnitId) ? 'selected' : '';
                    return `<option value="${u.id}" ${selected}>${u.unit_name}</option>`;
                }).join('');
            }

            function addItemRow(product) {
                // Prevent adding the same product twice - just focus its qty instead
                ...
                var defaultUnitId = product.product_unit_id || (product.units && product.units.length ? product
                    .units[0].id : '');
                var unitOptions = buildUnitOptions(product.units, defaultUnitId);
            ...
            <tr class="item-row" data-product-id="${product.id}">
                ...
                <td>
                    <select name="items[${i}][product_unit_id]" class="form-select form-select-sm unit-select">
                        ${unitOptions}
                    </select>
                </td>
            ...
            // Search products (reuses existing search-product endpoint)
