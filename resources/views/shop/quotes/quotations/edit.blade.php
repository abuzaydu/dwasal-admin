...
@section('content')
    ...
    <div class="row clearfix">
        <div class="col-md-10 mx-auto">
            <div class="card">
                <div class="card-header"><h5>Edit {{ $quotation->quote_number }}</h5></div>
                <div class="card-body">
                    <form action="{{ route('quotations.update', encrypt($quotation->id)) }}" method="POST" id="quotationForm">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="status" class="form-select">
                                    @foreach(['Draft','Sent','Accepted','Rejected','Expired'] as $status)
                                        <option value="{{ $status }}" {{ $quotation->status == $status ? 'selected' : '' }}>{{ $status }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                        ...
                        <table class="table table-sm" id="itemsTable">
                            ...
                            <tbody>
                                ...
                                <tr class="item-row">
                                    <td><input type="text" name="items[{{ $i }}][item_description]" class="form-control form-control-sm" value="{{ $item->item_description }}" required></td>
                        ...
                        <div class="row justify-content-end">
                            <div class="col-md-5">
                                <div class="row mb-2"><div class="col-6">Subtotal</div><div class="col-6 text-end" id="subtotalDisplay">{{ number_format($quotation->subtotal, 2) }}</div></div>
                                <div class="row mb-2"><div class="col-6">Discount</div><div class="col-6"><input type="number" step="0.01" min="0" name="discount" id="discountInput" class="form-control form-control-sm text-end" value="{{ $quotation->discount }}"></div></div>
                                <div class="row mb-2"><div class="col-6">Tax</div><div class="col-6"><input type="number" step="0.01" min="0" name="tax_amount" id="taxInput" class="form-control form-control-sm text-end" value="{{ $quotation->tax_amount }}"></div></div>
                                <div class="row mb-2 fw-bold"><div class="col-6">Total</div><div class="col-6 text-end" id="totalDisplay">{{ number_format($quotation->total, 2) }}</div></div>
                            </div>
                        </div>
