    ...
    <div class="block-header pt-1">
        <div class="row">
            ...
            <div class="col-lg-7 col-md-7 col-sm-12 text-right pt-0">
                <form class="dashform row g-3" action="{{ url('recycle-sales') }}" method="get" id="stockform">
                    @csrf
            ...
            <div class="col-md-12 tab-content py-3">
                <div class="tab-pane fade show active" id="manage-sales" role="tabpanel">
                    <div class="table-responsive">
                        <table id="empty-multiple" class="table table-striped display nowrap" style="width: 100%;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ number_format($sale->sale_amount - $sale->sale_discount - $sale->adjustment, 2, '.', ',') }}</td>
                                    <td>{{ number_format($sale->sale_amount_paid, 2, '.', ',') }}</td>
                                    <td>{{ number_format($sale->sale_amount - $sale->sale_discount - $sale->adjustment - $sale->sale_amount_paid, 2, '.', ',') }}</td>
                                    <td>{{ $sale->del_by }} </td>
                                    <td>{{ $sale->status }}</td>
                                    <td>
                                        <a href="#" class="button" onclick="confirmRecycle('<?php echo encrypt($sale->id); ?>')">
                                        <i class="fa fa-recycle"></i> Restore
                                        </a> | <a href="#" class="button"
                                            onclick="confirmDelete('<?php echo encrypt($sale->id); ?>')"><i class="fa fa-trash" style="color: red;"></i> Delete Parmanently</a>
                        ...
                        <form id="del-sales" action="{{ url('del-multiple-recycle-sales') }}" method="POST">
                            @csrf
                            <button id="submitDelButton" type="submit"class="btn btn-danger">{{ trans('navmenu.delete_selected') }}</button>
                            <button id="submitResButton" type="submit" class="btn btn-primary">{{ trans('navmenu.restore_selected') }}</button>
                       </form>
                    </div>
...
@section('page-scripts')
    ...
    <script type="text/javascript">
        ...
        $('#del-sales').on('submit', function(e){
            var form = this;
