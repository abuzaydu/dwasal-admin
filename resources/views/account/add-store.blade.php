        ...
        <div class="col-md-10 mx-auto">
            <div class="box box-success">
                <div class="card radius-6 p-4">
                    <form class="row g-3 needs-validation my-form" method="POST" action="{{ url('add-warehouse')}}">
                        @csrf
                        <div class="col-sm-6">
                            <label class="form-label">{{trans('navmenu.name')}} <span style="color:red">*</span></label>
                            <input id="shopname" type="text" name="shop_name"  data-rule="minlen:5" data-msg="{{trans('navmenu.hnt_enter_business_name')}}" class="form-control form-control-sm mb-1" placeholder="Enter Warehouse/Store Name" value="{{old('shop_name')}}" required>
                        </div>
                        <div class="col-sm-6">
                            ...
                            <select name="parent_shop_id" class="form-select form-select-sm mb-1">
                                @foreach (Auth::user()->shops()->where('is_warehouse', false)->get() as $key => $myshop)
