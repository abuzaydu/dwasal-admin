    ...
    <div class="block-header pt-4">
        <div class="row">
            ...
            <div class="col-lg-8 col-md-7 col-sm-12 text-right">
                <form class="dashform form-horizontal " action="{{url('f-prod-wips')}}" method="POST" id="stockform">
                    @csrf
                        ...
                        <li class="nav-item" role="presentation">
                            <a class="nav-link active" data-bs-toggle="tab" href="#tab_1-1" role="tab" aria-selected="true">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="tab-title">Work In Progress</div>
                                </div>
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link"  href="{{route('prod-wips.create')}}" >
                                <div class="d-flex align-items-center">
                                    <div class="tab-icon"><i class='fa fa-plus font-18 me-1'></i></div>
                                    <div class="tab-title">New Work In Progress</div>
                                </div>
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link" href="{{ url('prod-mwips') }}">
                                <div class="d-flex align-items-center">
                                    <div class="tab-icon"><i class='fa fa-tasks font-18 me-1'></i></div>
                    ...
                    <div class="tab-content py-3">
                        <div class="tab-pane fade show active" id="tab_1-1" role="tabpanel">
                            <div class="table-responsive">
                                <table id="del-multiple" class="table table-striped table-bordered display nowrap" style="width: 100%; font-size: 14px;">
                                    <thead style="font-weight: bold; font-size: 14;">
                                        <tr>
                                            ...
                                            <th>Item Name</th>
                                            <th>Opening WIP QTY</th>
                                            <th>Produced QTY</th>
                                            <th>Moved to Finished</th>
                                            <th>WIP Loss/Wastage</th>
                                            <th>Closing WIP QTY</th>
                                            <th>Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td style="text-align: center;">{{date('d-m-Y', strtotime($wip->date))}}</td>
                                            <td>{{$wip->name}}</td>
                                            <td style="text-align: center;">{{$wip->bf_balance}}</td>
                                            <td style="text-align: center;">{{$wip->produced}}</td>
                                            <td style="text-align: center;">{{$wip->finished_qty}}</td>
                                            <td style="text-align: center;">{{$wip->wip_damage}}</td>
                                            <td style="text-align: center;">{{$wip->closing_qty}}</td>
                                            <td>
                                                <a href="{{route('prod-wips.edit', encrypt($wip->id))}}">
                                                    <span class="fa fa-edit"></span>
                                                </a> |
                                                <form id="delete-form-{{$index}}" method="POST" action="{{route('prod-wips.destroy' , encrypt($wip->id))}}" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                <form id="frm-example" action="{{url('delete-multiple-wips')}}" method="POST">
                                    @csrf
