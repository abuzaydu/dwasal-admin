...
@section('content')
        ...
        <div class="row">
            <div class="col-lg-7 col-md-7 col-sm-12">
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ url('prod-labour-costs') }}">Labour Costs</a></li>
                    <li class="breadcrumb-item active">{{$title}}</li>
                </ul>
            </div>            
            <div class="col-lg-5 col-md-5 col-sm-12 text-right pt-0">
                <form class="dashform row" action="{{ url('f-plc-payments') }}" method="POST">
                @csrf
        ...
        <div class="card">
            <div class="card-body">
                <ul class="nav nav-tabs nav-tabs-new2">
                    ...
                    <a class="btn btn-success btn-sm" href="#" data-bs-toggle="modal" data-bs-target="#plcPayModal"><i class="fa fa-plus"></i> New Payment</a>
                </ul>
                    ...
                    <div class="tab-pane fade show active" id="tab_1-1" role="tabpanel">
                        <div class="table-responsive">
                            <table id="del-multiple" class="table table-striped table-bordered display nowrap" style="width: 100%; font-size: 14px;">
                                ...
                                <tbody>
                                    @foreach($plc_payments as $index => $plcpay)
                                    <tr>
                                        <td>{{$index+1}}</td>
                                        <td>{{$plcpay->pay_date}}</td>
                                        <td>{{ number_format($plcpay->amount)}}</td>
                                        <td>{{$plcpay->pay_mode}}</td>
                                        <td>{{$plcpay->first_name}}</td>
                                        <td>
                                            <a href="{{route('plc-payments.edit', encrypt($plcpay->id))}}">
                                                <i class="fa fa-edit" style="color: blue;"></i>
                                            </a>
                                            <form id="delete-form-{{$index}}" method="POST" action="{{route('plc-payments.destroy' , encrypt($plcpay->id))}}" style="display:inline;">
                                                 @csrf
                                                 @method('DELETE')
                        ...
                        <div class="table-responsive">
                            <table id="export-payments" class="table table-striped table-bordered display nowrap" style="width: 100%; font-size: 14px;">
                                ...
                                <tbody>
                                    <?php $totalpay = 0; ?>
                                    @foreach($plc_payments as $index => $plcpay)
                                    <?php $totalpay += $plcpay->amount; ?> 
                                    <tr>
                                        <td>{{ date('d/m/Y H:i:s', strtotime($plcpay->pay_date)) }}</td>
                                        <td>{{ number_format($plcpay->amount)}}</td>
                                        <td>{{$plcpay->pay_mode}}</td>
                                        <td>{{$plcpay->first_name}}</td>
                                    </tr>
    ...
    <div class="modal fade" id="plcPayModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="myModalLabel">New Labour Cost Payment</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form class="row g-1 form" method="POST" action="{{route('plc-payments.store')}}">
                    @csrf
                    <div class="col-md-3">
                    ...
                    <div class="col-sm-3" id="cashaccount" style="display: none;">
