            ...
            <div class="card">
                <div class="card-body">
                    <div class="row print_invoice">
                        <div class="col-md-4">
                            <form class="row g-3" action="{{route('plc-items.store')}}" method="POST">
                                @csrf
                                <input type="hidden" name="prod_labour_cost_id" value="{{$plc->id}}">
                                <div class="col-md-12">
                                    <label class="form-label">Add Item </label>
                                    <select name="production_stage_id" class="form-select form-select-sm mb-1" onchange='if(this.value != 0) { this.form.submit(); }' required>
                                ...
                                <table class="items mt-0"  style="width: 100%;  display: block; white-space: nowrap; overflow: auto;">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td style="text-align: center;">
                                                <form id="delete-form-{{$key}}" method="POST" action="{{route('plc-items.destroy' , encrypt($item->id))}}" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    <tfoot>
                                        <tr>
                                            ...
                                            <th style="text-align: center;"><b>{{ number_format($plc->amount, 2, '.', ',') }}</b></th>
                                            <th></th>
                                        </tr>
                                    </tfoot>
                                </table>
                        ...
                        <div class="col-md-12">
                            <form method="POST" action="{{ route('prod-labour-costs.update', encrypt($plc->id)) }}" class="row g-3 pt-3">
                                @csrf
                                {{ method_field('PATCH') }}
                                <div class="col-md-3">
                                    ...
                                    <div class="inner-addon left-addon">
                                        ...
                                        <input type="text" name="date" value="{{$plc->date}}" id="date" placeholder="{{trans('navmenu.pick_date')}}" class="form-control form-control-sm mb-3" required>
                                    </div>
                                </div>
                                <div class="col-md-8">
                                    <label class="form-label">Remarks</label>
                                    <input type="text" name="remarks" value="{{$plc->remarks}}" placeholder="Enter Your Remarks" class="form-control form-control-sm mb-3" required>
                                </div>
                                <div class="col-md-12">
                                    <button type="submit" name="myButton" class="btn btn-success btn-sm">{{trans('navmenu.btn_submit')}}</button>
                                    <a href="{{ url('prod-labour-costs') }}" class="btn btn-warning btn-sm float-end" style="margin-right: 5px;">Cancel</a>
                                </div>
                            </form>
                        </div>
                    </div>
