    ...
    <div class="block-header pt-4">
        <div class="row">
            ...
            <div class="col-lg-6 col-md-6 col-sm-12 text-right pt-0">
                <form class="dashform row" action="{{ url('f-moh-costs') }}" method="POST">
                    @csrf
                    ...
                    <!-- /.form group -->
                    <div class="col-md-4">
                        <a class="btn btn-primary btn-sm" href="{{ route('moh-costs.create') }}"><i class="fa fa-plus"></i> New MOH</a> 
                    </div>
                </form>
            </div>
        </div>
                    ...
                    <li class="nav-item" role="presentation">
                        <a class="nav-link active" data-bs-toggle="tab" href="#tab_1-1" role="tab" aria-selected="true">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="tab-title">MOH Costs</div>
                            </div>
                        </a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link" data-bs-toggle="tab" href="#tab_1-2" role="tab" aria-selected="true">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="tab-title"> Export MOH Costs</div>
                            </div>
                        </a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link" href="{{ url('moh-payments') }}">
                            <div class="d-flex align-items-center">
                                <div class="tab-icon"><i class='fa fa-money font-18 me-1'></i></div>
                                <div class="tab-title"> MOH Cost Payments</div>
                            </div>
                        </a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link" href="{{ url('mro') }}">
                            <div class="d-flex align-items-center">
                                <div class="tab-icon"><i class='fa fa-list-ol font-18 me-1'></i></div>
                                <div class="tab-title"> MOH Cost Items</div>
                            </div>
                        </a>
                    </li>
                </ul>
                    ...
                    <div class="tab-pane fade show active" id="tab_1-1" role="tabpanel">
                        <div class="table-responsive">
                            <table id="del-multiple" class="table table-striped table-bordered display nowrap" style="width: 100%; font-size: 14px;">
                                ...
                                <tbody>
                                    @foreach($mohcosts as $index => $moh)
                                    <tr>
                                        <td>{{$index+1}}</td>
                                        <td>{{ date('d/m/Y H:i:s', strtotime($moh->date)) }}</td>
                                        <td><a href="{{route('moh-costs.show', encrypt($moh->id))}}">{{ sprintf('%04d', $moh->moh_no) }}</a></td>
                                        <td>{{ number_format($moh->amount, 2, '.',',') }}</td>
                                        <td>{{ number_format($moh->amount_paid, 2, '.',',') }}</td>
                                        <td>{{ number_format($moh->amount-$moh->amount_paid, 2, '.',',') }}</td>
                                        <td>{{ $moh->created_at }}</td>
                                        <td>
                                            <a href="{{route('moh-costs.edit', encrypt($moh->id))}}">
                                                <i class="fa fa-edit" style="color: blue;"></i>
                                            </a>
                                            <form id="delete-form-{{$index}}" method="POST" action="{{route('moh-costs.destroy' , encrypt($moh->id))}}" style="display:inline;">
                                                 @csrf
                                                 @method('DELETE')
                        ...
                        <div class="table-responsive">
                            <table id="example2" class="table table-striped table-bordered display nowrap" style="width: 100%; font-size: 14px;">
                                ...
                                <tbody>
                                    <?php $total = 0; $tpaid = 0; ?>
                                    @foreach($mohcosts as $index => $moh)
                                    <?php $total += $moh->amount; $tpaid += $moh->amount_paid; ?>
                                    <tr>
                                        <td>{{ date('d/m/Y H:i:s', strtotime($moh->date)) }}</td>
                                        <td>{{ sprintf('%04d', $moh->moh_no) }}</td>
                                        <td>{{ number_format($moh->amount, 2, '.',',') }}</td>
                                        <td style="text-align: center;">{{ number_format($moh->amount_paid, 2, '.',',') }}</td>
                                        <td style="text-align: center;">{{ number_format($moh->amount-$moh->amount_paid, 2, '.',',') }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
