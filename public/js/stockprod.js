    ...
    app.controller("SearchItemCtrl", [ '$scope', '$http', '$filter', '$timeout', function($scope, $http, $filter, $timeout) {
        $scope.purchaseTempId = function(purchase_temp_id) {
            ...
            $scope.getData = function() {
                $http({
                    method: 'GET',
                    url: 'api/stockprodtemp/'+$scope.tempid
            ...
            $scope.addStockTemp = function(item) {
                $http({
                    method : 'POST',
                    url : 'api/stockprodtemp',
                    data : { purchase_temp_id: $scope.tempid, product_id: item.id, quantity_in: 0 } 
                }).then(function(response){
            ...
            $scope.updateStockTemp = function(newstocktemp) {
                ...
                $http({
                    method : 'PUT',
                    url : 'api/stockprodtemp/'+ newstocktemp.id ,
                    data: {quantity_in: newstocktemp.quantity_in, unit_cost: newstocktemp.unit_cost, total: newstocktemp.total, retail_price: newstocktemp.retail_price, expire_date: newstocktemp.expire_date} 
                }).then(function (response){
            ...
            $scope.sumQty = function(list) {
                var total=0;
                angular.forEach(list , function(newstocktemp){
                    total+= parseFloat(newstocktemp.quantity_in);
                });
                $scope.total_amount = total;
                return total;
            }
            ...
            $scope.addCostTemp = function(purchasetemp) {
                $http({
                    method : 'POST',
                    url : 'api/costtemp',
                    data : { purchase_temp_id: purchasetemp.id } 
                }).then(function(response){
                    $scope.getData();
                }, function(error){
                    console.log(error);
            ...
            $scope.updateCostTemp = function(newcosttemp) {
                $http({
                    method : 'PUT',
                    url : 'api/costtemp/'+ newcosttemp.id ,
                    data: {item_desc: newcosttemp.item_desc, percent: newcosttemp.percent, amount: newcosttemp.amount, total_amount: $scope.total_amount} 
                }).then(function (response){
                    $scope.getData();
                });
            }   

            $scope.removeCostTemp = function(id) {
                $http({
                    method: 'DELETE',
                    url: 'api/stockprodtemp/' + id ,
                }).then(function (response){
            ...
            $scope.sumCosts = function(list) {
                var total =0;
                angular.forEach(list , function(newcosttemp){
                    total += parseFloat(newcosttemp.amount);
                });
                return total;
            }

            $scope.sumPercentCosts = function(list) {
                var totalPercent =0;
                angular.forEach(list , function(newcosttemp){
                    totalPercent += parseFloat(newcosttemp.percent);
                });
                return totalPercent;
            }
