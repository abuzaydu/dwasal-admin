...
class DailyClosingStockJob implements ShouldQueue
{
    ...
    {
        $shop = $this->shop;
        Log::info('Checking closing stock for : '.$shop->name);
        ...
        foreach ($products as $key => $product) {
            ...
            if (is_null($checkdcs)) {
                dispatch(new StockUpdaterJob($shop, $product));
                ...
                if (!is_null($lastdcs)) {
                    ...
                    $start_retail_value = $lastdcs->end_retail_value;
                    $start_wholesale_value = $lastdcs->end_wholesale_value;
                }

                $start = $date.' 00:00:00';
                $end = $date.' 23:59:59';
                $purchase_qty = 0;
                $stocks = Stock::where('shop_id', $shop->id)->where('product_id', $product->id)->whereBetween('stock_date', [$start, $end])->where('is_deleted', false)->get();
                if ($stocks->count() > 0) {
                ...
                // Log::info('Item closing stock already created');
            }
        }
    }
}
